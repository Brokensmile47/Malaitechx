...
// ─── React with start emojis (fires in background, non-blocking) ─────────────
async function reactStart(sock, message, userMessage) {
    ...
    for (const emoji of emojis) {
        ...
        // Small delay between multi-step emojis (won't block command — called with .catch)
        if (emojis.length > 1) {
...
// ─── Diverse emoji pool for auto-reacting to every incoming message ───────────
...
function getRandomAutoReactEmoji() {
    return autoReactEmojis[Math.floor(Math.random() * autoReactEmojis.length)];
}

// ─── React to every incoming message if areact is enabled ─────────────────────
async function reactToEveryMessage(sock, message) {
    try {
        let areactEnabled = false;
        try {
            if (fs.existsSync(areactConfigPath)) {
                const data = JSON.parse(fs.readFileSync(areactConfigPath));
                areactEnabled = data.enabled;
            }
        } catch (_) {}

        if (!areactEnabled) return;

        // Don't react to our own messages
        if (message.key?.fromMe) return;

        const emoji = getRandomAutoReactEmoji();
        await react(sock, message, emoji);
    } catch (err) {
        // Silent fail — reactions are non-critical
    }
