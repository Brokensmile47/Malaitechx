 ...
 * - Built by Kimani Samuel (Brokensmile47)
 */
...
// ── Bot profile setter (inline — no external file needed) ─────────────────────
        ...
        for (const img of imgs) {
            ...
            if (fs.existsSync(full)) {
                try {
                    ...
                    console.log(chalk.green('✅ Bot profile picture set'));
                } catch (_) {}
                break;
            }
        }
...
// ── Channel info (used in all bot messages) ───────────────────────────────────
const malaiChannelInfo = {
    forwardingScore: 1,
    isForwarded: true,
...
async function startXeonBotInc() {
    try {
        ...
        // Global connection state — checked by autorecord and other presence features
        ...
        XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
            try {
                ...
                // NOTE: Do NOT clear msgRetryCounterCache here — breaks linked device sync
        ...
        XeonBotInc.ev.on('messages.update', async (updates) => {
            for (const update of updates) {
                if (update.update?.message) {
                    try { await store.loadMessage(update.key.remoteJid, update.key.id); } catch (_) {}
                }
            }
        });
        ...
        XeonBotInc.ev.on('message-receipt.update', async (updates) => {
            for (const { key } of updates) {
                try { await store.loadMessage(key.remoteJid, key.id); } catch (_) {}
            }
        });
        ...
        if (pairingCode && !XeonBotInc.authState.creds.registered) {
            ...
            if (!pn('+' + phoneNumber).isValid()) {
                console.log(chalk.red('Invalid phone number. Please enter full international number without + or spaces.'));
                process.exit(1);
            }
            ...
            setTimeout(async () => {
                try {
                    ...
                    console.log(chalk.bgGreen(chalk.black(`Your Pairing Code : `)), chalk.white(code))
        ...
        XeonBotInc.ev.on('connection.update', async (s) => {
            ...
            if (connection === 'open') {
                ...
                console.log(chalk.yellow(`║  🔗 Repo    : Malaitechx              ║`))
            ...
            if (connection === 'close') {
                ...
                console.log(chalk.red(`Connection closed — ${lastDisconnect?.error?.message || 'unknown reason'}`))
                ...
                if (statusCode === DisconnectReason.loggedOut || statusCode === 401) {
                    ...
                    console.log(chalk.red('Logged out. Please restart and pair again.'))
                }
